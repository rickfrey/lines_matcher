...
int main ( int argc, char *argv[] )
{
    ...
    while (!in.atEnd()) {
        ...
        if(numOfCol==6)
        {
            ...
            vector<float> posenvektor;//hier werden die 6 DoF reingespeichert
            QStringList list1 = line.split(" ");
            for(int i = 0; i < list1.size(); ++i)
            {
                posenvektor.push_back(list1.at(i).toFloat()); // Eingelesene Elemente werden in posenvektor geschrieben (und gleichzeitig zu int konvertiert)
            }
            bi.setPosenvektor(posenvektor); // posenvektor wird an Objekt an Posenvektor von Objekt bi übergeben
            //int Poseneintrag=bi.getPoseneintrag();
            //cout<<"Eintrag aus Posenvektor: "<<Poseneintrag<<endl;
    ...
    file.close();
    float test=BildInfoVector[3500].getPoseneintrag(0);
    cout<<"Poseneintrag[0]= "<< test << endl;
...
//      4. Gesamtfehler des Liniensets in jeweiligem Objekt speichern
      ////////////
    // reales Linienset laden
    for(int i = 0; i < 6; i++)// es werden 5 Zeilen reserviert
    {
        vector<int> row;
        reales_Set.push_back(row);
    }
